...
void CSimulation::RK4() {
	...
	for (vector<CPlanet>::iterator it = m_SS.m_planets.begin();
		...
		k1.push_back(it->getDynamicsPtr()->m_velocity * dt);
		i++;
	}
	...
	for (vector<CPlanet>::iterator it = m_SS.m_planets.begin();
			it < m_SS.m_planets.end(); it++) {
		new_SS.m_planets[i].getDynamics().m_position += k1[i] / 2;

		k2.push_back(CalcDv(dt / 2, new_SS.m_planets,i));
		i++;
	}
	...
	for (vector<CPlanet>::iterator it = m_SS.m_planets.begin();
			it < m_SS.m_planets.end(); it++) {
		new_SS.m_planets[i].getDynamics().m_position += k2[i] / 2;
	...
	for (vector<CPlanet>::iterator it = m_SS.m_planets.begin();	it < m_SS.m_planets.end(); it++) {
		new_SS.m_planets[i].getDynamics().m_position += k3[i];
	...
	for (vector<CPlanet>::iterator it = m_SS.m_planets.begin();
		...
		m_SS.m_planets[i].getDynamicsPtr()->m_position = it->getDynamicsPtr()->m_position+ (k1[i] + k2[i]*2 +k3[i]*2 + k4[i]) * (1.0 / 6.0);
	...
	sum = zero;
	 ...
	 for (vector<CPlanet>::iterator it = m_SS.m_planets.begin();it < m_SS.m_planets.end(); it++) {
		 ...
		 if(dist > 0){
			 ...
			 sum += r_vec*(it->m_mass)/pow(dist,3);
		 }
	 }
